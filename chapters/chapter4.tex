...
$p$所相关的表面点个数$C_p$。然后我们根据$C_p$来调整粒子的质量，对于每一个$p\in \mathcal{V}$，$\tilde{m}_p = \frac{m_p}{2C_p + 1}$，同时我们分配给表面粒子和嵌入粒子质量分别为
$\tilde{m}_{s_p} = \tilde{m}_p$，$\tilde{m}_{o_p} = \tilde{m}_p$,同样我们赋予两种粒子当前$t^n$时刻的速度为$v_s^n = v_{o_s}^n = v_{p_s}^n$。
\begin{figure}[htbp]
    \centering
...
因此网格上的速度场更新为
\begin{align}
    \hat{v}_{\mathbf{j}}^{n+1} = \frac{1}{m_\mathbf{j}^n}(m_{\mathbf{j}}^nv_{\mathbf{j}}^n + \Delta t f^{liquid}_{\mathbf{j}} + \Delta t f^{surface}_{\mathbf{j}})
\end{align}
\subsection{计算流程}
