...
假设我们在网格上更新了信息之后，为了将网格上的解算结果返回到粒子上，我们需要网格信息到粒子的过程(G2P)。最直观的做法是直接使用
样条基函数在粒子位置上取值来传递信息，即
\begin{align*}
    v^{n+1}_p &= \sum_\mathbf{i} \hat{v}^{n+1}_{\mathbf{i}}B_\mathbf{i}(x_p^n)\\
    \nabla v_p^{n+1} &= \sum_\mathbf{i} \hat{v}^{n+1}_{\mathbf{i}}\nabla B_\mathbf{i}(x_p^n)    
\end{align*}
然而，以上的传递方式损失了太多的速度信息，实际上我们只保留了速度在粒子上的的零阶信息，这样的结果最终将导致模拟出的流体结果过粘[**]，
其中一种解决方案[**]为在粒子上储存更高阶的速度信息，如我们额外的在每个粒子$p$上使用一个矩阵$A_p$来保存速度的梯度，即保存速度的一阶信息。
