...
\subsection{水滴自由下落}
该实验表面张力系数$k = 8.0$，球体由80000粒子表示。该实验总耗用内存约为14GB，每个时间步长平均为0.7487s，每一帧由50个时间步长组成，
...
\subsection{拓扑变化}
该实验表面张力系数$k = 2.0$，球体由60000粒子表示。该实验耗用内存约为12GB，每个时间步长平均为0.8523s，每一帧由150个时间步长组成，
平均一帧耗时为122秒。水滴初速度为0.9，初始高度为0.2，自由下落。可以发现在触及光滑表面后出现拓扑变化，然后再表面张力的影响下慢慢恢复，具体实验场景见\ref{fig: sphere splash experience}。
\begin{figure}[htbp]
    \centering
    \includegraphics[scale=0.7]{./images/DropSplashEXP.png}
    \caption[水滴拓扑变化实验]{水滴拓扑变化}
    \label{fig: sphere splash experience}
\end{figure}
