...
使用粒子水平集方法~\cite{boyd2012multiflip}获得一个粗糙的$C^0$连续隐式曲面，并使用Marching Cube~\cite{heiden1993fast}算法重构表面网格。然而该网格
的质量只能做到$C^0$连续，计算得到的法向在面片之间并不连续，难以应用在表面张力所需的法向计算。本文在该网格
上进行泊松圆盘采样，然后结合已经存在的IPIA算法~\cite{hamza2020implicit}，提出一种新的快速重建算法以此获得一个二阶光滑
...
从实验结果的图\ref{fig: MarchingCube vs after Sparse-LSIPIA}(2)中可以发现，Marching cube算法确实提取出了一个点云的边界，但是轮廓只能做到$C^0$连续，法向明显不连续，同时其三角网格质量无法
...
则$B_{ijk}(v_r) = B_{ijk}(q_r) = 0$，我们可以去掉该节点，同时去掉对应的$\mathbf{B}$和$\mathbf{C}$对应的行，其并不影响点云附近的隐式曲面构建。所以我们不妨假定$\Lambda$可逆。在有了这个假设之后，我们进一步发现，我们可以将问题规模大大减小。
...
\begin{figure}[htbp]
    ...
    \caption[稀疏数据结构]{带状点云结构，绿色分段直线为Marching cube输出网格，黑色为采样点云，红色光滑曲线拟合结果，半透明红色为黑色点云周围的网格。}
    \label{fig: SparseSparse-LSIPIA}
\end{figure}

式(3.7)给出的迭代格式收敛性的证明，详见LSPIA~\cite{lin2018convergence}。本迭代格式(下称为Sparse-LSIPIA)相较于IPIA格式最大的优势就是内存占用少，可高度并行，我们从\textbf{Algorithm \ref{alg:Sparse-LSIPIA}}来分析这一点。
...
图\ref{fig: MarchingCube vs after Sparse-LSIPIA}(3)为使用Sparse-LSIPIA拟合点云后的结果，可以发现拟合结果明显更光滑。图\ref{fig: Sparse-LSIPIA_experience}展示了更多的实验结果，图中实验皆选自Yusuf等人~\cite{hamza2020implicit}在2019年耗时较长的实验，在原文中最短的兔子实验耗时为88.24秒，其他三个模型均在900秒以上，
...
\begin{figure}[htbp]
    \centering
    \includegraphics[scale=0.60]{./images/LSIPIA_experience.png}
    \caption[Sparse-LSIPIA实验结果]{Sparse-LSIPIA实验图，其中左上角模型为bunny模型实验，左上角第一张图为bunny点云输入，第二张为Sparse-LSIPIA拟合结果。右上角，左下角，右下角分别为budahha模型，armadillo模型，fertility模型的实验结果}
...
\subsection{快速法向计算}
在完成\textbf{Algorithm \ref{alg:Sparse-LSIPIA}}之后，我们获得了一个二阶连续的隐式曲面。即
\begin{equation}
...
格点张量积B-样条基下，移动加权最小二乘的结果将有惊人的简单形式。

我们在空间中每一点$y$寻求一个在$y$周围的函数来拟合原始函数$f$，近似目标函数如下
\begin{equation}
    f_y (x;c) = f(y) + c^T(x - y)    
\end{equation}
