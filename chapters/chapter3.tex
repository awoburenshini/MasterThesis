...
\begin{figure}[htbp]
    ...
    \caption[提取网格对比]{图(1)为输入点云,图(2)为直接提取网格结果,图(3)为LSIPIA拟合后提取的网格结果}
    \label{fig: MarchingCube vs after LSIPIA}
\end{figure}


从实验结果的图\ref{fig: MarchingCube vs after LSIPIA}(2)中可以发现，Marching cube算法确实提取出了一个点云的边界轮廓，但是轮廓只能做到一阶连续，法向明显不连续，同时其三角网格质量无法
...
\end{equation}
求解该线性系统，我们使用迭代法求解，与IPIA不同，我们的迭代格式为
\begin{equation}
...
所以我们不妨假定$\Lambda$可逆。有关收敛性的证明，详见LSPIA~\cite{lin2018convergence}。本迭代格式(下称为LSIPIA)相较于IPIA格式最大的优势就是内存占用少，可高度并行，我们从\textbf{Algorithm \ref{alg:LSIPIA}}来分析这一点。
...
图\ref{fig: MarchingCube vs after LSIPIA}(3)为使用LSIPIA拟合点云后的结果,可以发现拟合结果明显更光滑。图\ref{fig: LSIPIA_experience}更多的实验结果，图中实验皆选自Yusuf等人~\cite{hamza2020implicit}在2019年耗时较长的实验，在原文中最短的兔子实验耗时为88.24秒，其他三个模型均在900秒以上。
...
\begin{figure}[htbp]
    ...
    \caption[LSIPIA实验结果]{LSIPIA实验图，其中左上角模型为bunny模型实验，左上角第一张图为bunny点云输入，第二张为LSIPIA拟合结果。右上角，左下角，右下角分别为budahha模型，armadillo模型，fertility模型的实验结果}
...
可以发现要计算$x$点的梯度$\nabla f$, 我们需要计算的有$$B_i(x^r),\partial_r B_i(x^r), i \in \{0,1,2\}, r \in \{1,2,3\}$$其中样条阶数越高，计算$\partial_r B_i(x^r)$代价越大，并且$\partial_r B_i(x^r)$为分段函数,故在难以在$r$之间不可能做到单指令多数据流并行，同样在拼装
